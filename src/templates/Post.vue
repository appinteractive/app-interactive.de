    ...
    <main>
      ...
      <article class="max-w-xl md:max-w-2xl xl:max-w-3xl mx-auto px-6 sm:px-12 pt-16" :class="{'border-b border-grey-lighter pb-10 mb-16': !$page.post.author}">
        ...
        <footer v-if="$page.post.author || $page.post.tags" class="flex flex-wrap pb-10 sm:pb-16">
          <div>
            <g-link v-for="tag in $page.post.tags" :key="tag.id" :to="`${tag.path}/`" class="inline-block text-gray-700 hover:text-gray-800 bg-gray-200 border border-gray-300 hover:border-gray-400 font-sans text-xs sm:text-sm px-4 py-2 mr-4 mb-2 rounded transition-color transition-border">
